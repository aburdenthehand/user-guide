...
RUN mkdir -p /src && cd /src && \
    ...
    pip install mkdocs mkdocs-material mkdocs-awesome-nav mkdocs-htmlproofer-plugin mkdocs-redirects mkdocs-static-i18n mike && \
    gem list && \
    ...
    dnf remove -y @development-tools gcc qt5-srpm-macros \
      ...
      diffutils npm pigz bzip2 xz jq -x git -x make && \
    dnf clean all && \
